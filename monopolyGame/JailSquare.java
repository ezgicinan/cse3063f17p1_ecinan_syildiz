...
public class JailSquare extends Square {
	...
	public void squareAction(Player player, Board board) {
		...
		if(board.previousPosition == 30 || board.threetimesDoubled==1){ 	//if player is sent from GoToJail square or 
																			//come here by rolling 3 times doubles enter loop 
			//Print where does the player go
			System.out.printf("three times doubled= %d\n", board.threetimesDoubled);
			System.out.printf("%s is in %s\n", player.getName(), this.name);
			System.out.printf("You're come from Square 30 or you rolled 3 times Doubles, So you are %s now.\n ", this.name);
			...
			System.out.printf("%s is Just Visiting\n", player.getName());
		}
		
		board.threetimesDoubled=0;
